...
ggplot(inla.joint.samps, aes(R, n)) + stat_density_2d(
  ...
  contour = FALSE) +
  geom_point(data = data.frame(R = R_fun_1(fit$summary.fixed$mode[2]), n = alpha_fun_1(fit$summary.fixed$mode[3])), color = 'red', shape = 1, size =0.5) + 
  coord_fixed() + scale_fill_viridis_c(name = 'Density') + theme_minimal() + xlab('$R$') + ylab('$n$') + xlim(c(2, 12)) + ylim(c(0.15, 4))
dev.off()
...
eval_func <- function(par){
  ...
  form <- coordinates ~ Intercept + beta1*exp(-(gal1/R1^2)^(1/a1)) + field
...
system.time(
  res_opt <- BO_adap_optim(eval_func, update_step = 5, number_eval = 100, D = 2, 
                           lower = c(2, 0.15), upper = c(12, 4), length_scale = 0.5, signal_var = 50, num_initial = 10, delta = 0.00005)
)

# save BOSS results. 
saveRDS(res_opt, 'BO_UDG_apps_100.RDS')
...
res_opt <- readRDS('BO_UDG_apps_100.RDS')
...
unique_data$y_cen <- unique_data$y - mean(unique_data$y)
...
surrogate <- function(xvalue, data_to_smooth, cov){
  predict_gp(data_to_smooth, x_pred = xvalue, choice_cov = cov, noise_var = 1e-10)$mean
}
...
x.1 <- (seq(from = 2, to = 12, length.out = 500) - 2)/10
x.2 <- (seq(from = 0.15, to = 4, length.out = 500) - 0.15)/3.85
...
# normalize
lognormal_const <- log(sum(exp(fn_vals))*0.02*0.0077)
...
ggplot(post_x, aes(x.1,x.2)) + geom_raster(aes(fill = pos)) + 
  ...
  coord_fixed() + scale_fill_viridis_c(name = 'Density') + theme_minimal() + xlab('$R$') + ylab('$n$')
dev.off()
...
# get posterior marginal samples of (R, n) from inlabru
set.seed(1234)
...
R_marginal <- data.frame(R = c(inla.samples.a, samples[,1]), 
                         method = rep(c('inlabru', 'BOSS (ours)'),  c(length(inla.samples.a), length(samples[,1]))))
...
n_marginal <- data.frame(n = c(inla.samples.b, samples[,2]), 
                            method = rep(c('inlabru', 'BOSS (ours)'),  c(length(inla.samples.b), length(samples[,2]))))

# plot marginal of n
tikz(file = "n_marginal_UDG.tex", standAlone=T, width = 4, height = 2)
ggplot(n_marginal, aes(n)) + geom_density(aes(color = method)) + theme_minimal() + xlab('$n$')
dev.off()
system('pdflatex n_marginal_UDG.tex')

# posterior distribution of spatial random field U through inlabru
U <- predict(fit, pixels(v_mesh, nx = 300, ny = 300, mask = region), ~ exp(field), seed = 1234)
...
ff$fn(pnorm(as.numeric(unique_data[which.max(unique_data$y), c(1,2)])))
ff$gr(pnorm(as.numeric(unique_data[which.max(unique_data$y), c(1,2)])))
...
ggplot(exceed_map, aes(x,y)) + geom_raster(aes(fill = prob)) + 
  scale_fill_distiller(palette = 'Spectral', name = '$\\mathbb{P}(\\mathcal{U}(s) > C \\mid \\mathbf{X})$') +
...
U_0.5 <- data.frame(BO_aghq_int[[1]]$U[,c('coords.x1','coords.x2')])
...
U_dat$method <- rep(c('BOSS (ours)', 'inlabru'), each = 40486)
